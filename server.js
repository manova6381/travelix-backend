...
app.post("/api/create/destination", (request, response) => {
  const sql_query = `INSERT INTO mano_destinations (destinationName , location , destinationImage , destinationCount) VALUES ('${request.body.destinationName}', '${request.body.location}', '${request.body.destinationImage}', '${request.body.destinationCount}')`;
...
app.get("/api/list/destination", (request, response) => {
  const sql_query = `SELECT * FROM mano_destinations`;
  connection.query(sql_query, (error, result) => {
    if (error) {
      response.status(500).send(error);
    } else {
...
app.delete("/api/delete/destination/:id", (request, response) => {
  const sql_query = `DELETE FROM mano_destinations WHERE id=${request.params.id}`;
  connection.query(sql_query, (error, result) => {
    if (error) {
      response.status(500).send(error);
    } else {
...
app.post("/api/create/hotel", (request, response) => {
  const sql_query = `INSERT INTO mano_hotels (name, destination, image, price, location, available) VALUES ('${request.body.name}', '${request.body.destination}', '${request.body.image}', '${request.body.price}',  '${request.body.location}', ${request.body.available})`;
...
app.get("/api/load/hotels", (request, response) => {
  const sql_query = `SELECT * FROM mano_hotels`;
  connection.query(sql_query, (error, result) => {
    if (error) {
      response.status(500).send(error);
    } else {
...
app.get("/api/search/destination", (request, response) => {
  ...
  let sql_query = `SELECT * FROM mano_destinations WHERE destinationName LIKE '${destinationName}%' AND location LIKE '${destinationLocation}%'`;
...
app.get("/api/search/hotel", (request, response) => {
  ...
  const sql_query = `SELECT * from mano_hotels WHERE destination LIKE '${destinationName}%'`;
